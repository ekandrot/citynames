...
for city in cities:
    if len(city) < 3:
        continue
    letter = city[0]
    firstLetter[letter] = firstLetter.get(letter, 0) + 1
...
def generate_with_combos2():
    for i in range(20):
        ...
        letter = findnth(firstLetter, r)
        word += letter
    
        r = int(random.uniform(0, secondLetter[letter]['total']-1))
        letter = findnth(secondLetter[letter], r)
        word += letter
    
        while letter != '\n':
            r = int(random.uniform(0, combos2[letter]['total']-1))
            letter = findnth(combos2[letter], r)
            if letter == '\n':
                break
            word += letter
        print(word)
...
def generate_with_combos3():
    for i in range(20):
        ...
        while letter != '\n':
            ...
            letter = findnth(combos3[prevCombo], r)
            if letter == '\n':
                break
            word += letter
            prevLetter0, prevLetter1 = prevLetter1, letter
